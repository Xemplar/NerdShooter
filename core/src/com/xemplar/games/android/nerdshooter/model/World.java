...
public class World {
    ...
    public Array<Extra> getExtras(int width, int height) {
        ...
        Array<Extra> extras = new Array<Extra>();

        for(Extra e : level.getExtras()){
            Vector2 pos = e.getPosition();
            if(x <= pos.x && pos.x < x2 && y <= pos.y && pos.y <= y2) extras.add(e);
        }
    ...
    public Array<Block> getVisibleBlocks(int width, int height) {
        int x = (int)jaxon.getPosition().x - width;
        int y = (int)jaxon.getPosition().y - height;
        if (x < 0) {
            x = 0;
    ...
    public World(String pack, int levelNum){
        EntityController.setWorld(this);
